...
public class MongoTemplate implements MongoOperations, ApplicationContextAware, IndexOperationsProvider {
	...
	public <T> T findOne(Query query, Class<T> entityClass, String collectionName) {
		return findOne( query,  entityClass,  collectionName,null);
	}

	@Nullable
	public <T> T findOne(Query query, Class<T> entityClass, String collectionName,ReadPreference readPreference) {
		...
		if (ObjectUtils.isEmpty(query.getSortObject()) && !query.getCollation().isPresent()) {
			return doFindOne(collectionName, query.getQueryObject(), query.getFieldsObject(), entityClass,readPreference);
		} else {
			query.limit(1);
			List<T> results = find(query, entityClass, collectionName,readPreference);
	...
	public <T> List<T> find(Query query, Class<T> entityClass, String collectionName,ReadPreference readPreference) {
		...
		return doFind(collectionName, query.getQueryObject(), query.getFieldsObject(), entityClass,
				new QueryCursorPreparer(query, entityClass),readPreference);
	}
	...
	public <T> T findById(Object id, Class<T> entityClass, String collectionName) {
		return findById(id,  entityClass,  collectionName,null);
	}

	@Nullable
	public <T> T findById(Object id, Class<T> entityClass, String collectionName,ReadPreference readPreference) {
		...
		return doFindOne(collectionName, new Document(idKey, id), new Document(), entityClass,readPreference);
	}
	...
	public long count(Query query, Class<?> entityClass,ReadPreference readPreference) {
		Assert.notNull(entityClass, "Entity class must not be null!");
		return count(query, entityClass, getCollectionName(entityClass),readPreference);
	}
	...
	public long count(final Query query, String collectionName,ReadPreference readPreference) {
		return count(query, null, collectionName,readPreference);
	}
	...
	public long count(Query query, @Nullable Class<?> entityClass, String collectionName) {
		...
		return count(query, entityClass, collectionName,null);
	}


	public long count(Query query, @Nullable Class<?> entityClass, String collectionName,ReadPreference readPreference) {
		...
		return doCount(collectionName, document, options,readPreference);
	}


	protected long doCount(String collectionName, Document filter, CountOptions options) {

		return doCount(collectionName,  filter,  options,null);
	}

	protected long doCount(String collectionName, Document filter, CountOptions options,ReadPreference readPreference){
		...
		if (MongoDatabaseUtils.isTransactionActive(getMongoDbFactory())) {
			return execute(collectionName, collection -> collection.countDocuments(filter, options,readPreference));
		}

		return execute(collectionName, collection -> collection.count(filter, options,readPreference));
	}
	...
	public <T> List<T> findAll(Class<T> entityClass, String collectionName) {
		return findAll(entityClass,collectionName,null);
	}

	public <T> List<T> findAll(Class<T> entityClass, String collectionName,ReadPreference readPreference) {
		return executeFindMultiInternal(new FindCallback(new Document(), new Document()), null,
				new ReadDocumentCallback<>(mongoConverter, entityClass, collectionName), collectionName,readPreference);
	}
	...
	protected <T> T doFindOne(String collectionName, Document query, Document fields, Class<T> entityClass) {
		return doFindOne(collectionName,  query,  fields, entityClass,null);
	}

	protected <T> T doFindOne(String collectionName, Document query, Document fields, Class<T> entityClass,ReadPreference readPreference) {
		...
		return executeFindOneInternal(new FindOneCallback(mappedQuery, mappedFields),
				new ReadDocumentCallback<>(this.mongoConverter, entityClass, collectionName), collectionName,readPreference);
	}
	...
	protected <T> List<T> doFind(String collectionName, Document query, Document fields, Class<T> entityClass,
								 CursorPreparer preparer,ReadPreference readPreference) {
		return doFind(collectionName, query, fields, entityClass, preparer,
				new ReadDocumentCallback<>(mongoConverter, entityClass, collectionName),readPreference);
	}
	...
	protected <S, T> List<T> doFind(String collectionName, Document query, Document fields, Class<S> entityClass,
		...
		return doFind(collectionName,  query,  fields,entityClass, preparer,  objectCallback,null);
	}

	public <S, T> List<T> doFind(String collectionName, Document query, Document fields, Class<S> entityClass,
									@Nullable CursorPreparer preparer, DocumentCallback<T> objectCallback,ReadPreference readPreference) {
	...
	<S, T> List<T> doFind(String collectionName, Document query, Document fields, Class<S> sourceClass,
		...
		return doFind(collectionName,  query,  fields,sourceClass,
				 targetClass,  preparer,null);
	}
	 ...
	 * @since 2.0
	 */
	public  <S, T> List<T> doFind(String collectionName, Document query, Document fields, Class<S> sourceClass,
						  Class<T> targetClass, CursorPreparer preparer,ReadPreference readPreference) {
		...
		return executeFindMultiInternal(new FindCallback(mappedQuery, mappedFields), preparer,
				new ProjectingReadCallback<>(mongoConverter, sourceClass, targetClass, collectionName), collectionName,readPreference);
	}
	...
	private <T> T executeFindOneInternal(CollectionCallback<Document> collectionCallback,
		...
		return executeFindOneInternal( collectionCallback,
				objectCallback,  collectionName,null);
	}

	@Nullable
	private <T> T executeFindOneInternal(CollectionCallback<Document> collectionCallback,
										 DocumentCallback<T> objectCallback, String collectionName,ReadPreference readPreference) {

		try {
			T result = objectCallback.doWith(collectionCallback.doInCollection(getAndPrepareCollection(doGetDatabase(), collectionName),readPreference));
			return result;
		} catch (RuntimeException e) {
			throw potentiallyConvertRuntimeException(e, exceptionTranslator);
		}
	...
	private <T> List<T> executeFindMultiInternal(CollectionCallback<FindIterable<Document>> collectionCallback,
			@Nullable CursorPreparer preparer, DocumentCallback<T> objectCallback, String collectionName,ReadPreference readPreference) {
		...
		try {
			...
			try {
				...
				cursor = iterable.iterator(readPreference);
	...
	private static class FindOneCallback implements CollectionCallback<Document> {
		...
		public Document doInCollection(MongoCollection<Document> collection) throws MongoException, DataAccessException {
			return doInCollection(collection,null);
		}
		@Override
		public Document doInCollection(MongoCollection<Document> collection,ReadPreference readPreference) throws MongoException, DataAccessException {
			...
			return iterable.first(readPreference);
		}
	}
